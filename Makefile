...
## Source: https://github.com/armory-io/go-makefile/resources/templates/Makefile.hbs
## Build timestamp: 20230427082323
##
#################
...
VERSION_MARKER        = "20230427082323"

#####################
## Build steps
#####################
.PHONY: all
all: bootstrap-project clean build-dirs run-before-tools build check run-after-tools
...
bootstrap-project:
	@test ! -f "$(GEN_DIR)/.makefile_$(VERSION_MARKER)" && \
	...
	mkdir -p "$(GEN_DIR)" && \
	touch -f "$(GEN_DIR)/.makefile_$(VERSION_MARKER)" || echo latest makefile already exists
