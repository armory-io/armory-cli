...
func buildTemplateKubernetesCore() *yaml.Node {
	...
	// Target root
	targetNode, targetValuesNode := util.BuildMapNode("targets","Map of deployment targets. You can specify more than one target, but we currently only support deploying to one. Specifying more than one target will be available in a future feature.")
	...
	path2.Content = append(path2.Content, util.BuildStringNode("path", "infrastructure/manifests/deployment.yaml",
		"This will read the specified file and deploy the manifests in it to all targets.")...)
