...
func NewCmdRoot(outWriter, errWriter io.Writer) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "armory",
		Short: "A CLI for using Armory CDaaS",
	}

	addr := rootCmd.PersistentFlags().StringP("addr", "", "https://api.cloud.armory.io", "")
	rootCmd.PersistentFlags().MarkHidden("addr")

	clientId := rootCmd.PersistentFlags().StringP("clientId", "c", "", "configure oidc client credentials for Armory CDaaS API")
	clientSecret := rootCmd.PersistentFlags().StringP("clientSecret", "s", "", "configure oidc client credentials for Armory CDaaS API")
